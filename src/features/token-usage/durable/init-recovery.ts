...
import { normalizeEventDeltas } from './projector.ts'
import type { SqliteUsageStore } from './sqlite-store.ts'
...
export class InitRecoveryCoordinator {
  ...
  async scan(): Promise<void> {
    ...
    if (phase === 'initializing') {
      ...
      // Clean/ready startup: sweep sessions no collector ever ingested —
      // created while no token-usage instance was running (usage in profiles
      // without a collector, e.g. gui/CLI runs before this feature existed).
      // Bootstrapped sessions cost one indexed checkpoint lookup each.
      await this.runGapSweep(this.snapshots)
    }
  }
  ...
  /**
   * Project sessions that exist in the log but never completed ingestion.
   * Keeps the store phase as-is (live serving continues); failures bump
   * failedSessions without degrading — the sweep re-runs on the next clean
   * boot, resuming from each session's committed checkpoint.
   */
  private async runGapSweep(
    snapshots: ReadonlyArray<{ header: { id: string; createdAt?: number; cwd?: string }; revision: unknown }>,
  ): Promise<void> {
    if (this.aborted) return
    let failed = 0
    for (const snapshot of snapshots) {
      ...
      if (pk !== undefined) {
        const checkpoint = await this.store.getCheckpoint(pk)
        if (checkpoint.bootstrapComplete) continue
        resumeFrom = checkpoint.lastSeq + 1
      }
      try {
        await this.scanLifecycle(snapshot, resumeFrom, true)
      } catch (error) {
        failed += 1
        console.error('token-usage: gap sweep failed for session', identity.sessionId, error)
      }
      await new Promise((resolve) => setImmediate(resolve))
    }
    if (failed > 0) {
      await this.store.updateProjectionProgress({ failedSessions: failed }, this.now())
    }
  }
