...
export class UsageWorkerClient {
  ...
  constructor(options: WorkerClientOptions) {
    ...
    // `type: 'module'` is missing from @types/node's WorkerOptions but
    // accepted at runtime; without it node 20 runs the ESM bundle as a
    // classic script and the worker fails to parse.
    this.workerFactory = options.workerFactory ?? (() => new Worker(new URL('./usage-worker.js', import.meta.url), { type: 'module' } as never) as unknown as WorkerLike)
